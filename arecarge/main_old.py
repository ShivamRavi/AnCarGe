...
from arecarge.ui.pages import gui_home, done_page as dope, boss_page, inventory_page, distracted_page as disp, ability_page
...
def use_ability(player):
    ...
    ability = input("Enter ability: ").lower()
...
def use_ask(player):
    ...
    choice = input("Use from stash or backpack? (stash/backpack/ability): ").strip().lower()
    if choice == "stash":
        player.use_item()
...
def check_for_msg_squeue(msg="start"):
...
def cli_input_loop():
    ...
    tester_mode = name.strip() == "Tester"
    if tester_mode:
        print("Tester mode onini bananini!")

    if os.path.exists(f"{name}_progress.json"):
        if input("Save file found. Load it? (yes/no): ").lower() in ("yes", "y", "yea", "yuppi duppi", "oki") and not tester_mode:
    ...
    while True:
            ...
            command = input("Enter command (or 'help'/'exit'/'use'): ").strip().lower()
...
def main():
    ...
    def poll_queue():
        try:
            while True:
                msg = sq.get_nowait()
                if not switcher.current_page in [gui_home.HomePage, boss_page.BossPage, inventory_page.InventoryPage]:
